...
`docs` is the docs-lite Hugo site repo in the current multi-repo stack.

### Module identity

- Module path: `github.com/dan-hill/docs`

### Dependency chain

`docs` imports `github.com/dan-hill/site-base`, and `site-base` imports `github.com/dan-hill/ox`.

That means the visual stack for `docs` is:

- `docs` → `site-base` → `ox`

### Local override

`docs/layouts/page.html` remains a local override of the shared page template and contains the marker:

- `DOCS LOCAL OVERRIDE ACTIVE`

This override should continue to affect docs regular pages only.
...
3. `docs` is updated to the intended `site-base` tag with:
